...
const Fillter = memo(({ className, params, setParams, visible, setVisible, row, setRow, data }) => {
  ...
  const pushQuery = useMemo(() => {
    ...
    if (_.size(params.phone) > 0) {
      pathPhone = JSON.stringify(params.phone);
    }
    if (_.size(params.status) > 0) {
      pathStatus = JSON.stringify(params.status);
    }
    history.push({
      pathname: pathName,
      search: `?start=${pathStartDay}&end=${pathEndDay}&name=${pathFullName}&phone=${pathPhone}&status=${pathStatus}`,
    });
  }, [params]);
  ...
  useEffect(() => {
    ...
    const paramPhone = query.get("phone");
    ...
    setParams((preState) => {
      ...
      if (paramPhone) {
        nextState.phone = JSON.parse(paramPhone);
      }
      if (paramStatus) {
        nextState.status = JSON.parse(paramStatus);
      }
      return nextState;
    });
  }, [pushQuery]);
      ...
      <Col xl={6} lg={6} md={6} xs={6}>
        ...
        <Col xxl={4} xl={4} lg={4} md={4} sm={4}>
          <Input
              ...
              onChange={(value) => {
                getQuery(value.target.value, "phone");
              }}
            />
        </Col>
        <Col xxl={4} xl={4} lg={4} md={4} sm={4}>
